...
function __prim_stringIteratorNext(str, it) {
  if (it >= str.length) return { h: 0 };
  else return { a1: str.charAt(it), a2: it + 1 };
}
...
const _truncToChar = x =>
  String.fromCodePoint((x >= 0 && x <= 55295) || (x >= 57344 && x <= 1114111) ? x : 0);
...
const _shr32u = (a, b) =>
  _truncUInt32(
    a <= 0x7fffffff ? a >> b : b == 0 ? a : (a >> b) ^ (-0x80000000 >> (b - 1))
  );
